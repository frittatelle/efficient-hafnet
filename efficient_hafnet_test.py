...
if __name__ == '__main__':
    ...
    rgb_dir = 'data/Vaihingen/RGB_patches'
    dsm_dir = 'data/Vaihingen/DSM_patches'
    labels_dir = 'data/Vaihingen/Labels_patches'
    ...
    gamma = 0.1
    steps = batch_size
    tr_perc = 0.2 #to get an integer number of samples divisible by steps
            ...
            if (batch_idx % steps == 0 and batch_idx != 0) or (batch_idx % steps == 0 and epoch > 0):
                ...
                with torch.no_grad():
                    for val_batch_idx, val_batch in enumerate(val_loop):
                        ...
                        val_loss += loss.item()
                        val_iou += iou(pred, label, threshold=0.5)
                        val_fscore += f_score(pred, label, threshold=0.5)
